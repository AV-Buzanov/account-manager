...
public class AccountService implements IAccountService {
    ...
    public AccountService(@NotNull final AccountRepository accountRepository,
                          @NotNull UserRepository userRepository, @NotNull final TransactionRepository transactionRepository,
    ...
    public Collection<AccountDto> findAll(int page, int size, final User user) throws Exception {
//        return userRepository.findById(userId)
//                .orElseThrow(Exception::new).getAccounts().stream()
//                .skip(page*size)
//                .limit(size)
...
//                .collect(Collectors.toList());
    ...
    @Nullable
    @Transactional
    public AccountDto create(@Nullable final AccountDto accountDto, @Nullable final User user) throws Exception {
    ...
    @Nullable
    @Transactional
    public boolean addUser(@Nullable final String id, String username) throws Exception {
        if (id == null || id.isEmpty() || username == null || username.isEmpty())
            throw new Exception("Argument can't be empty or null");
        Account findedAccount = accountRepository.findById(id).orElseThrow(Exception::new);
        User user = userRepository.findByUsername(username).orElseThrow(Exception::new);
        findedAccount.getUsers().add(user);
        return accountRepository.saveAndFlush(findedAccount).getUsers().contains(user);
    }
